...
static uint8_t mrfiCurrentLogicalChannel = 2; /* Default logical channel */
...
//Ghaith: modify FRMCTRL0 to change ENERGY_SCAN if maximum RSSI is wanted instead of last one
...
void Mrfi_FiFoPIsr(void)
{
 ...
 // uint8_t ret =0;
  ...
  /*ret = MRFI_ASSERT( mrfiRadioState == MRFI_RADIO_STATE_RX ); //deepa
  if(ret == 0)
  {
  	flush/ dummy read fifo
  }*/
   ...
   */
   if(!MRFI_FIFOP_STATUS() || !MRFI_FIFOP_STATUS()) {
  //count_mrfi(); //calls a function that increments a counter by one to minitor number of times 
                //that this function hase been called 
  MRFI_CLEAR_RX_INTERRUPT_FLAG(); 
  return;
  }
