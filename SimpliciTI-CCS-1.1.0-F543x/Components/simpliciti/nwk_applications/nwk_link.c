 ...
 */
#ifdef ACCESS_POINT 
extern volatile linkID_t sLID[NUM_CONNECTIONS];
extern volatile uint8_t AppConnState[NUM_CONNECTIONS];
#endif
/******************************************************************************
 * GLOBAL FUNCTIONS
 */
...
static void smpl_send_unlink_reply(mrfiPacket_t *frame)
{
  ...
  if (pCInfo = nwk_findPeer((addr_t *)MRFI_P_SRC_ADDR(frame), *(MRFI_P_PAYLOAD(frame)+F_APP_PAYLOAD_OS+UL_RMT_PORT_OS)))
  {
    ...
    {
    	if(sLID[i] == pCInfo->thisLinkID) break;
    }
    AppConnState[i] = CONNSTATE_FREE;
#endif    
    rc = SMPL_SUCCESS;
  }
